...
1. **Setup Phase** - Launches Chrome with anti-detection settings
2. **Navigation** - Opens the target URL and waits for page load
3. **Extraction** - Locates movie table rows and parses each field
4. **Storage** - Writes results to timestamped files
5. **Logging** - Records run summary with status
...
1. Set `"use_proxy": true` in config.json
2. Add your proxy URL: `"proxy_url": "http://user:pass@proxy:port"`
3. Consider using a rotating residential proxy service
...
*Built for automated film data collection and analysis*
